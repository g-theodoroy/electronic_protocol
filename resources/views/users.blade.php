                ...
                <form name="myForm" class="form-horizontal" role="form" method="POST" action="{{ url('/users') }}{{$user->id ? '/' . $user->id : ''}}">
                    ...
                    <div class='row'>
                        <div class="col-md-7 col-sm-7 {{ $errors->has('name') ? ' has-error' : '' }}">
                            <input id="name" type="text" class="form-control" name="name" placeholder="name" value="{{ old('name') ? old('name') : $user->name }}" required autofocus >
                        </div>
                        <div class="col-md-5 col-sm-5 {{ $errors->has('username') ? ' has-error' : '' }}">
                            <input id="username" type="text" class="form-control" name="username"  placeholder="username" value="{{ old('username') ? old('username') : $user->username }}" required >
                        </div>
                    </div>
                    </div>
                        <div class="col-md-2 col-sm-2 {{ $errors->has('email') ? ' has-error' : '' }}">
                            <input id="email" type="email" class="form-control" name="email"  placeholder="email" value="{{ old('email') ? old('email') : $user->email }}" required >
                        </div>
                        <div class="col-md-3 col-sm-3 {{ $errors->has('password') ? ' has-error' : '' }}">
                            <div class='row'>
                                <div class="col-md-6 col-sm-6">
                                    <input id="password" type="password" class="form-control col-md-1 col-sm-1" name="password" placeholder="passwd"  required >
                                </div>
                                <div class="col-md-6 col-sm-6">
                                    <input id="password-confirmation" type="password" class="form-control  col-md-1 col-sm-1" name="password_confirmation" placeholder="confirm"  required >
                                </div>
                            </div>
                        </div>
